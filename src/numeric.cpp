...
TEST_CASE("iota", CATCH_CAN_SKIP_THIS + sTestCase_Numeric)
{
	...
	BENCHMARK("iota")
				{
					std::iota(vSeries.begin(), vSeries.end(), 1);
				};
...
TEST_CASE("accumulators", CATCH_CAN_SKIP_THIS + sTestCase_Numeric)
{
	...
	BENCHMARK("accumulate - add")
				{
					std::accumulate(vSeries.cbegin(), vSeries.cend(), 1, std::plus{});
				};

	BENCHMARK("accumulate - multiply")
				{
					std::accumulate(vSeries.cbegin(),
					                vSeries.cend(),
					                1,
					                std::multiplies{});
				};
	BENCHMARK("reduce - add")
				{
					std::reduce(vSeries.cbegin(), vSeries.cend(), 1, std::plus{});
				};

	BENCHMARK("reduce - multiply")
				{
					std::reduce(vSeries.cbegin(), vSeries.cend(), 1, std::multiplies{});
				};
}
